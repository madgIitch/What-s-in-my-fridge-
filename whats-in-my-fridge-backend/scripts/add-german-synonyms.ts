 ...
 *
 * Uso: npx ts-node scripts/add-german-synonyms.ts
 */
...
const BATCH_SIZE = 10; // Procesar en lotes para no sobrecargar
const DELAY_MS = 500; // Delay entre llamadas para evitar rate limiting
...
async function getGermanTranslation(ingredient: string): Promise<string[]> {
...
German translation:`;
  ...
  try {
    ...
    const translations = result
      .split(',')
      .map((t: string) => t.trim())
      .filter((t: string) => t.length > 0 && t.length < 50) // Filtrar respuestas inválidas
      .filter((t: string) => !t.includes('translation') && !t.includes(':')) // Filtrar texto extra
      .slice(0, 3); // Máximo 3 traducciones

    return translations;
  } catch (error) {
    console.error(`❌ Error traduciendo "${ingredient}":`, error);
    return [];
  }
}
...
async function main() {
  ...
  const vocabulary: VocabularyFile = JSON.parse(fileContent);
  ...
  for (let i = 0; i < ingredients.length; i += BATCH_SIZE) {
    ...
    for (const [key, data] of batch) {
      ...
      // Verificar si ya tiene sinónimos alemanes (heurística simple)
      const hasGerman = data.synonyms.some(syn =>
        /[äöüß]/.test(syn) || // Tiene caracteres alemanes
        ['zwiebel', 'milch', 'reis', 'fleisch', 'käse', 'brot', 'ei', 'pilz'].some(de => syn.includes(de))
      );
      ...
      if (germanTranslations.length > 0) {
        // Filtrar traducciones que ya existen
