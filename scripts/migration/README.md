...
node scripts/migration/data/run.mjs storage-plan --environment staging --source-project what-s-in-my-fridge-a2a07 --source-bucket what-s-in-my-fridge-a2a07.appspot.com --snapshot C:\secure\fridge-s12-snapshot --report-dir C:\secure\fridge-s12-reports --confirm-source-read READ_FIRESTORE_LEGACY
```
...
`storage-plan` reads the confirmed Firebase bucket and compares objects under `receipts/` with explicit paths stored in snapshot fields `storagePath`, `imagePath`, or `receiptImagePath`. It reports referenced, missing, and orphan objects using hashes; it never copies objects. The legacy `uploadReceipt` function returns a signed image URL but does not persist the receipt object path in its Firestore draft, so objects from that path are expected to appear as orphans unless another document explicitly references them. Confirm the actual bucket name before running the command; the tool accepts only the two bucket names associated with the confirmed project.
