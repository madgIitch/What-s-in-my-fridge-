...
export default async function AuthenticatedAppLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return <PrivateSessionGate userId={user.id}><PwaControls installPromotion={process.env.PWA_INSTALL_PROMOTION_ENABLED !== "false"} pushEnabled={process.env.PUSH_ENABLED !== "false" && Boolean(process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY)} /><AppBackLink /><div className="app-content">{children}</div></PrivateSessionGate>;
}
